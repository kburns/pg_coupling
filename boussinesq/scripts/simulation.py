...
# Parent internal wave
p['g'] =  parent_amp * np.sin(kx*x + kz*z) * kz          / K**2
b['g'] =  parent_amp * np.cos(kx*x + kz*z)
u['g'] =  parent_amp * np.sin(kx*x + kz*z) * kx * kz / ω / K**2
w['g'] = -parent_amp * np.sin(kx*x + kz*z) * kx**2   / ω / K**2

# Buoyancy perturbations on grid
gshape = domain.dist.grid_layout.global_shape(scales=1)
gslices = domain.dist.grid_layout.slices(scales=1)
rand = np.random.RandomState(seed=42)
noise = rand.standard_normal(gshape)[gslices]
b['g'] += noise_amp * noise

# Spectrally filter buoyancy perturbations
local_K = np.sqrt((global_kx**2 + global_kz**2)[cslices])
b['c'] *= (1 + np.tanh(noise_cut - local_K)) / 2
